...
? : 그냥 생성자에 다 때려 박으면 되는거 아닌가요??
-> *객체의 생성과 초기화를 분리합시다!!!*

* 싱글톤 빈은 스프링 컨테이너의 생성 시점에 메서드가 실행되지만 프로토타입 스프링 빈은 스프링 컨테이너에서 빈을 조회할 때 생성되고, 초기화 메서도도 실행된다.
* 싱글톤 빈은 스프링 컨테이너가 관리하기 때문에 스프링 컨테이너가 종료될 때 빈의 종료 메소드가 실행되지만, 프로토타입 빈은 스프링 컨테이너가 생성과 의존관계 주입, 그리고 초기화 까지만 관여하고 이후 관리하지 않는다. 따라서 프로토타입 빈은 스프링 컨테이너가 종료될 때 @PreDestroy 같은 종료 메소드가 실행 되지 않는다.

> 프로토타입 빈
* 스프링 컨테이너가 요청할 때 마다 새로 생성
* 스프링 컨테이너는 프로토타입 빈의 생성과 의존관계 주입, 그리고 초기화까지만 관여한다.
* 종료 메소드가 호출되지 않는다.


그러면 싱글톤 빈이 프로토타입 빈을 사용한다면 어떻게 매번 새로운 객체를 받을 수 있을까?
-> ObjectProvider<>을 사용하자
ObjectProvider의 getObject()를 호출하면 내부에서는 스프링 컨테이너를 통해 해당 빈을 찾아서 반환한다.(“Dependency Lookup”)


-> Provider를 사용하자 (JSP-330) : provider.get()
---
## 웹 스코프
* 웹 환경에서만 동작
* 해당 스코프의 종료 시점까지 관리를 해서 종료 메소드가 호출된다.

종류
* Request : HTTP 요청 하나가 들어오고 나갈 때 까지 유지되는 스코프, 요청마다 별도의 빈 인스턴스가 생성
* Session : HTTP Session과 동일 생명 주기
* Application : 서블릿 컨텍스트와 동일 생명 주기
* Websocket : 웹 소켓과 동일한 생명 주기

*UUID* : Universally Unique Identifier : 사용자를 구분하기 위해 사용하는 ID, 겹칠 일이 없다

* @Scope(“request”) : 빈은 HTTP 요청 당 하나씩 생성되고, HTTP 요청이 끝나는 시점에 종료된다.

아무것도 추가하지 않고 @Scope(“request”)를 사용하고 실행하면 오류가 날 것이다.
WHY? : 스프링 애플리케이션을 실행하는 시점에 싱글톤 빈은 생성해서 주입이 가능하지만, request 스코프 빈은 아직 생성되지 않는다(조회 시 생성!!). 이 빈은 실제 고객의 요청이 와야 생성할 수 있다.

해결방안
1. Provider : provider를 사용해서 빈을 요청하는 시간을 지연시키자.
2. ProxyMode : 프록시모드로 설정하면 내가 원하는 클래스의 가짜 프록시 클래스를 만들어두고 HTTP request와 상관 없이 가짜 프록시 클래스를 다른 빈에 미리 주입할 수 있다.
* Scope(proxyMode = ScopedProxyMode.TARGET_CLASS) : 스프링 컨테이너는 CGLIB 라이브러리를 사용해서, 내가 만든 클래스를 상속받은 가짜 프록시 객체를 생성한다.(다형성의 원리!)
* *가짜 프록시 객체는 요청이 오면 그 때 내부에서 진짜 빈을 요청하는 위임 로직이 있다*

---

### 이번 강의를 들으면서...

순수한 자바 코드로 짠 다음 객체 지향의 원칙을 지키는 것과 스프링 프레임워크가 제공하는 자동 기능을 통해 객체 지향의 원칙을 지키는 것에는 굉장한 차이가 있는 것 같다.

또한 스프링이 개발자들이 개발하기 편하게 최대한 *객체 지향*의 방향을 유지하면서 기능을 제공하는 것이 인상 깊었다.

모든 기능에 DI(I0C)나 OCP 등의 개념이 전반적으로 다 적용이 되어있는 것 같다.

처음에 스프링 입문 + MVC 강의를 들었을 때 Controller -> Service -> Repository 패턴으로 짜고 @이런거를 붙이면 알아서 다 연결되는 과정이 잘 이해가 안가고 단순히 스프링이 관리를 한다 정도로만 이해했는데, 이번 강의를 통해 좀 더 깊은 스프링에 대해 배우면서 *왜, 어떻게* 해당 기능들을 지원하는지에 대해 학습해서 다시 추후에 MVC 패턴과 같은 것들을 배울 때 보다 수월할 것 같다. 

이번에 학습하면서 초점을 둔 포인트는 **why** 였다. 해당 기술이 왜 등장하고, 왜 이러한 개념을 적용시키고, 왜 사용하는지 등이 있는 것 같다.

스프링을 사용하는 이유는 개인적으로 다음과 같이 생각한다.
1. 자바와 같은 객체 지향 프로그래밍의 근본적인 원칙을 준수하기 위한 개발자의 부담을 덜어준다
2. 스프링 컨테이너, 빈 등을 통해 객체 관리를 보다 수월하게 해주고 유지보수를 용이하게 해주어 애플리케이션의 확장성에 기여를 해준다.

 아직 백엔드 언어로 다른 것들을(파이썬,,) 사용해보지는 않았지만 스프링을 배우면서 객체 지향 프로그래밍의 매력에 빠져들어 계속 스프링을 사용할 것 같다.
